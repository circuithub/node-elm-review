...
async function get(options, elmJsonDependencies, pathToElmJson) {
  ...
  const dependencyAction =
    fsReadJson(pathToElmJson)
        .then((file) => {
          console.log("getting dependencies from the current elm.json");
          return file.dependencies
        });

  const npxElmJsonAction =
    spawnAsync(
      'npx',
      [
        '--no-install',
  ...
  return (options.offline ? dependencyAction : npxElmJsonAction)
    .then((dependencies) => {
...
function handleInternetAccessError(options, error) {
